...
class CultCinemaClassicsSkill(CommonPlaySkill):
    ...
    def __init__(self):
        ...
        # load video catalog
        videos = Collection("CultCinemaClassics",
    ...
    def initialize(self):
        self.add_event('skill-ccc.jarbasskills.home',
                       self.handle_homescreen)
        self.gui.register_handler(
            "skill-ccc.jarbasskills.play_event",
                                  self.play_video_event)
        self.gui.register_handler(
            "skill-ccc.jarbasskills.clear_history",
    ...
    @intent_file_handler('ccchome.intent')
    ...
    def handle_homescreen(self, message):
        ...
        self.gui["historyModel"] = JsonStorageXDG("ccc-history")\
            .get("model", [])
    ...
    def add_to_history(self, video_data):
        # History
        historyDB = JsonStorageXDG("ccc-history")
    ...
    def CPS_match_query_phrase(self, phrase, media_type):
        ...
        self.log.debug("Best CultCinemaClassics Tags Match: {s}, {t}".format(
            s=best_score, t=best_video["title"]))

        # match video name
        match, title_score, best_title, leftover_text = self.match_title(
            videos, phrase, match)
        self.log.debug("Best CultCinemaClassics Title Match: {s}, {t}".format(
        ...
        candidates = scores[:n]
        self.log.info("Choosing randomly from top {n} CultCinemaClassics matches".format(
            n=len(candidates)))
        ...
        self.log.debug("Best CultCinemaClassics video: " + best_video["title"])
